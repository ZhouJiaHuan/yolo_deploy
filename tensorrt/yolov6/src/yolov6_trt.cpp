...
void Yolov6TRT::nmSortedboxes(const std::vector<Object>& objects,
...
void Yolov6TRT::drawObject(cv::Mat& img, const std::vector<Object>& objects,
...
Yolov6TRT::Yolov6TRT(const TRTDetConfigs& trtCfg): trtCfg_(trtCfg)
{    
    initTRT();
}
...
cv::Mat Yolov6TRT::preProcess(cv::Mat& src)
{
...
void Yolov6TRT::postProcess(const float* output, std::vector<Object>& objects,
    const cv::Size& imgSize)
{
    int classNum = trtCfg_.classNum;
    std::vector<Object> proposals;
    int numProposal = outputSize_ / (classNum + 5);
    for (int idx=0; idx<numProposal; ++idx)
    {
        const int basicPos = idx *(classNum + 5);
        float cx = output[basicPos+0];
        float cy = output[basicPos+1];
        float w = output[basicPos+2];
        float h = output[basicPos+3];
        float score = output[basicPos+4];
        int clsId = 0;
        for (int id=0; id < classNum; ++id)
        {
            if (output[basicPos+5+id] > output[basicPos+5+clsId])
            {
                clsId = id;
            }
        }

        float prob = score * output[basicPos+5+clsId];
...
void Yolov6TRT::blobFromImage(float* blob, cv::Mat& img)
{
    int H = img.rows;
    int W = img.cols;
...
void Yolov6TRT::detect(cv::Mat& img, std::vector<Object>& objects)
{
    const int& inputW = trtCfg_.inputSize.width;
    const int& inputH = trtCfg_.inputSize.height;
