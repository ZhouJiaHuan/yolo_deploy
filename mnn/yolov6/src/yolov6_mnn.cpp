...
void Yolov6MNN::nmSortedboxes(const std::vector<Object>& objects,
...
void Yolov6MNN::drawObject(cv::Mat& img, const std::vector<Object>& objects,
...
Yolov6MNN::Yolov6MNN(const MNNDetConfigs& mnnCfg): mnnCfg_(mnnCfg)
{    
    ...
    const float norms[3] = {mnnCfg_.norms[0]/255, mnnCfg_.norms[1]/255, mnnCfg_.norms[2]/255};
...
cv::Mat Yolov6MNN::preProcess(cv::Mat& src)
{
...
void Yolov6MNN::postProcess(MNN::Tensor const* output, std::vector<Object>& objects,
    const cv::Size& imgSize)
{
    std::vector<Object> proposals;
    int numProposal = output->shape()[1];
    int classNum = mnnCfg_.classNum;
    auto basicPos = output->host<float>();
    for (int idx=0; idx<numProposal; ++idx)
    {
        const float* pos = basicPos + idx * (classNum + 5);
        float cx = pos[0];
        float cy = pos[1];
        float w = pos[2];
        float h = pos[3];
        ...
        {
            if (pos[5+id] > pos[5+clsId])
            {
                clsId = id;
            }
        }

        float prob = score * pos[5+clsId];
...
void Yolov6MNN::detect(cv::Mat& img, std::vector<Object>& objects)
{
    const int& inputW = mnnCfg_.inputSize.width;
    const int& inputH = mnnCfg_.inputSize.height;
